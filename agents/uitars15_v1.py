...
class UITARS15Agent:
    def __init__(
        self,
        model: str, # vllm serve --served-model-name $MODEL_NAME
        runtime_conf: Dict,
        platform="ubuntu",
    ...
    def predict(
        ...
        this_task_logger.info(f"Number of images in messages: {image_num}")
        try:
        ...
        top_k = self.top_k
        # place holder in case of the failure of the vllm client
        usage = {
            "prompt_tokens": 0,
            "completion_tokens": 0,
            "total_tokens": 0,
        ...
        while True:
            this_task_logger.info(f"Remaining Attempts: {try_times}, with temperature: {temperature}")
            if try_times <= 0:
                this_task_logger.error(f"Reach max retry times to fetch response from client, as error flag.")
                return "client error", ["DONE"], usage
            try:
                ...
                this_task_logger.error(f"Error when fetching response from client: {e}")
                prediction = None
                try_times -= 1
            ...
            try:
                ...
                this_task_logger.error(f"Error when parsing response from client: {e}")
        ...
        try:
            ...
            this_task_logger.error(f"Parsing action error: {prediction}, with error:\n{e}")
        ...
        for parsed_response in parsed_responses:
            if "action_type" in parsed_response:
                ...
                if parsed_response["action_type"] == FINISH_WORD:
                    ...
                    return prediction, ["DONE"], usage
                
                elif parsed_response["action_type"] == WAIT_WORD:
                    self.actions.append(actions)
                    return prediction, ["WAIT"], usage
                
                elif parsed_response["action_type"] == ENV_FAIL_WORD:
                    self.actions.append(actions)
                    return prediction, ["FAIL"], usage
        ...
        return prediction, actions, usage