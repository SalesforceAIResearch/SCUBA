...
def push_oss(image_path):
    ...
    raise NotImplementedError("OSS is not implemented")
...
class Grounding(BaseModule):
    ...
    def predict(self, ref_expr, image_list):
        ...
        response_raw = agent.get_response()
        response = response_raw.choices[0].message.content
        usage = response_raw.usage.to_dict()
        agent.add_message(text_content=response, role="assistant")
        numericals = re.findall(r"\d+", response)
        assert len(numericals) >= 2
        return [int(numericals[0]), int(numericals[1])], agent.messages, usage
...
class Manager(BaseModule):
    ...
    def predict(self, prompt, image_list):
        ...
        usage = response_raw.usage.to_dict()
        agent.add_message(text_content=response, role="assistant")
        return response, agent.messages, usage
