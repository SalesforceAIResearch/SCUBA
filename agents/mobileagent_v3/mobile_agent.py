...
import copy
import json
from agents.mobileagent_v3.mobile_agent_modules import (
    InfoPool,
    Manager,
    Executor,
    Grounding,
...
def convert_fc_action_to_json_action_grounding(
        ...
        if 'element_description' in action_json:
            [x, y], grounding_messages, grounding_usage = grounding_model.predict(grounding_info+action_json['element_description'], image_list)
        elif "element1_description" in action_json:
            [x1, y1], grounding_messages1, grounding_usage1 = grounding_model.predict(grounding_info+action_json['element1_description'], image_list)
            [x2, y2], grounding_messages2, grounding_usage2 = grounding_model.predict(grounding_info+action_json['element2_description'], image_list)
            grounding_messages = [grounding_messages1, grounding_messages2]
            grounding_usage = grounding_usage1
            for key in grounding_usage2:
        ...
        return JSONAction(
                ...
                ), grounding_messages, grounding_usage
...
class MobileAgentV3:
    ...
    def step(self, instruction: str, env, args, this_task_logger):
        ## init agents ## 
        ...
        # this_task_logger.info('----------step ' + str(step_idx + 1))
        ...
        if not skip_manager:
            ...
            output_planning, message_manager, manager_usage = manager.predict(prompt_planning, [before_screenshot])
            ...
            total_usage["prompt_tokens"] += manager_usage["prompt_tokens"]
            total_usage["completion_tokens"] += manager_usage["completion_tokens"]
            total_usage["total_tokens"] += manager_usage["total_tokens"]
        ...
        if "Finished" in self.info_pool.current_subgoal.strip():
            ...
            total_usage["completion_tokens"] += operator_usage["completion_tokens"]
            total_usage["total_tokens"] += operator_usage["total_tokens"]
            ...
            if (not action_thought) or (not action_object_str):
                this_task_logger.error('Action prompt output is not in the correct format.')
...
{action_description}'''
        ...
        try:
            if args.grounding_stage > 0:
                ...
                total_usage["completion_tokens"] += grounding_usage["completion_tokens"]
                total_usage["total_tokens"] += grounding_usage["total_tokens"]
                if grounding_messages is not None:
                    global_state['grounding'] = {
                        'name': 'grounding',
                        'messages': grounding_messages,
                        'parsed_response': asdict(converted_action)
                    }
            else:
            ...
            this_task_logger.error(str(e))
            self.info_pool.last_action = {"action": "invalid"}
            self.info_pool.action_history.append({"action": "invalid"})
            self.info_pool.summary_history.append(action_description)
            self.info_pool.action_outcomes.append("C") # no change
            self.info_pool.error_descriptions.append("invalid action format, do nothing.")
            return global_state, action_object_str, False, None, False, total_usage
        ...
        if converted_action.action_type == 'done':
            ...
            return global_state, converted_action.action_code, True, None, True, total_usage
        try:
            ...
            this_task_logger.error('Failed to execute action.')
            this_task_logger.error(str(e))
        ...
        this_task_logger.info("Done action execution.\n")
        self.info_pool.last_action = json.loads(action_object_str)
        ...
        if converted_action.action_type != 'answer':
            ...
            output_action_reflect, message_reflector, reflector_usage = reflector.predict(prompt_action_reflect, [before_screenshot, after_screenshot])
            
            total_usage["prompt_tokens"] += reflector_usage["prompt_tokens"]
            total_usage["completion_tokens"] += reflector_usage["completion_tokens"]
            total_usage["total_tokens"] += reflector_usage["total_tokens"]
        ...
        return global_state, converted_action.action_code, True, env_reward, env_done, total_usage
