"""
Adapted from https://github.com/xlang-ai/OSWorld/blob/main/mm_agents/opencua_agent.py
...
# AGNET_SYS_PROMPT_L2 = "You are a GUI agent. You are given a task and a screenshot of the screen. You need to perform a series of pyautogui actions to complete the task.\n\nFor each step, provide your response in this format:\n\nThought:\n  - Step by Step Progress Assessment:\n    - Analyze completed task parts and their contribution to the overall goal\n    - Reflect on potential errors, unexpected results, or obstacles\n    - If previous action was incorrect, predict a logical recovery step\n  - Next Action Analysis:\n    - List possible next actions based on current state\n    - Evaluate options considering current state and previous actions\n    - Propose most logical next action\n    - Anticipate consequences of the proposed action\n  - For Text Input Actions:\n    - Note current cursor position\n    - Consolidate repetitive actions (specify count for multiple keypresses)\n    - Describe expected final text outcome\n  - Use first-person perspective in reasoning\n\nAction:\n  Provide clear, concise, and actionable instructions:\n  - If the action involves interacting with a specific target:\n    - Describe target explicitly without using coordinates\n    - Specify element names when possible (use original language if non-English)\n    - Describe features (shape, color, position) if name unavailable\n    - For window control buttons, identify correctly (minimize \"—\", maximize \"□\", close \"X\")\n  - if the action involves keyboard actions like 'press', 'write', 'hotkey':\n    - Consolidate repetitive keypresses with count\n    - Specify expected text outcome for typing actions\n\nFinally, output the action as PyAutoGUI code or the following functions:\n- {\"name\": \"computer.triple_click\", \"description\": \"Triple click on the screen\", \"parameters\": {\"type\": \"object\", \"properties\": {\"x\": {\"type\": \"number\", \"description\": \"The x coordinate of the triple click\"}, \"y\": {\"type\": \"number\", \"description\": \"The y coordinate of the triple click\"}}, \"required\": [\"x\", \"y\"]}}\n- {\"name\": \"computer.terminate\", \"description\": \"Terminate the current task and report its completion status\", \"parameters\": {\"type\": \"object\", \"properties\": {\"status\": {\"type\": \"string\", \"enum\": [\"success\", \"fail\"], \"description\": \"The status of the task\"}}, \"required\": [\"status\"]}}\n- {\"name\": \"computer.terminateWithText\", \"description\": \"Terminate the current task and return an answer addressing the user question\", \"parameters\": {\"type\": \"object\", \"properties\": {\"text\": {\"type\": \"string\", \"description\": \"Free-form text answer addressing the user question\"}}, \"required\": [\"text\"]}}".strip()
AGNET_SYS_PROMPT_L2 = "You are a GUI agent. You are given a task and a screenshot of the screen. You need to perform a series of pyautogui actions to complete the task.\n\nFor each step, provide your response in this format:\n\nThought:\n  - Step by Step Progress Assessment:\n    - Analyze completed task parts and their contribution to the overall goal\n    - Reflect on potential errors, unexpected results, or obstacles\n    - If previous action was incorrect, predict a logical recovery step\n  - Next Action Analysis:\n    - List possible next actions based on current state\n    - Evaluate options considering current state and previous actions\n    - Propose most logical next action\n    - Anticipate consequences of the proposed action\n  - For Text Input Actions:\n    - Note current cursor position\n    - Consolidate repetitive actions (specify count for multiple keypresses)\n    - Describe expected final text outcome\n  - Use first-person perspective in reasoning\n\nAction:\n  Provide clear, concise, and actionable instructions:\n  - If the action involves interacting with a specific target:\n    - Describe target explicitly without using coordinates\n    - Specify element names when possible (use original language if non-English)\n    - Describe features (shape, color, position) if name unavailable\n    - For window control buttons, identify correctly (minimize \"—\", maximize \"□\", close \"X\")\n  - if the action involves keyboard actions like 'press', 'write', 'hotkey':\n    - Consolidate repetitive keypresses with count\n    - Specify expected text outcome for typing actions\n\nFinally, output the action as PyAutoGUI code or the following functions:\n- {\"name\": \"computer.triple_click\", \"description\": \"Triple click on the screen\", \"parameters\": {\"type\": \"object\", \"properties\": {\"x\": {\"type\": \"number\", \"description\": \"The x coordinate of the triple click\"}, \"y\": {\"type\": \"number\", \"description\": \"The y coordinate of the triple click\"}}, \"required\": [\"x\", \"y\"]}}\n- {\"name\": \"computer.terminate\", \"description\": \"Terminate the current task and report its completion status\", \"parameters\": {\"type\": \"object\", \"properties\": {\"status\": {\"type\": \"string\", \"enum\": [\"success\", \"fail\"], \"description\": \"The status of the task\"}}, \"required\": [\"status\"]}}\n- {\"name\": \"computer.terminateWithText\", \"description\": \"Terminate the current task by returning an answer addresses the user question and the task status\", \"parameters\": {\"type\": \"object\", \"properties\": {\"text\": {\"type\": \"string\", \"description\": \"Free-form text answer addressing the user question\"}, \"status\": {\"type\": \"string\", \"enum\": [\"success\", \"fail\"], \"description\": \"The status of the task\"}}, \"required\": [\"text\", \"status\"]}}}}".strip()
...
def parse_response_to_cot_and_action(input_string, screen_size, coordinate_type) -> Tuple[str, List[str], dict]:
    ...
    try:
        ...
        if "computer.terminateWithText" in input_string.lower():
            # Look for code blocks that might contain terminate command
            code_blocks = re.findall(r'```(?:code|python)?\s*(.*?)\s*```', input_string, re.DOTALL | re.IGNORECASE)
            if code_blocks:
                last_code = code_blocks[-1].strip().lower()
                candiate_answer = re.search(r"text\s*=\s*'([^']*)'", last_code)
                if candiate_answer:
                    answer = candiate_answer.group(1)
                else:
...
def transform_agnet_action_to_code_block(action):
    ...
    if "computer.terminate" in action or "browser.select_option" in action or "browser.clear" in action or "computer.terminateWithText" in action:
        return f"```code\n{action}\n```"
    else:
        return f"```python\n{action}\n```"
...
class OpenCUAAgent:
    ...
    def predict(self, 
        ...
        response, usage = self.call_llm_ray(payload, args, this_task_logger)

        # this_task_logger.info(f"Model Output: \n{response}")
        if not response:
            this_task_logger.error("No response found in the response.")
            return "ERROR", ["DONE"], usage
        ...
        # this_task_logger.info(f"Parsed Low-level Action: \n{low_level_instruction}")
        # this_task_logger.info(f"Parsed pyautogui Action: \n{pyautogui_actions}")

        self.actions.append(low_level_instruction)
        if 'action' not in other_cot or not other_cot['action'] or 'thought' not in other_cot or not other_cot['thought']:
            this_task_logger.error("Error! no action/thought in cot")
            this_task_logger.error(f"response: {response}")
            this_task_logger.error(f"cot: {other_cot}")
        ...
        this_task_logger.info(f"New step cot: {other_cot}")

        return response, pyautogui_actions, usage
    ...
    def call_llm_ray(self, payload: dict, args: argparse.Namespace, this_task_logger: logging.Logger):
        """Call the LLM ray service"""
        try_times = args.max_retry_per_request
        _usage = {
            "prompt_tokens": 0,
            "completion_tokens": 0,
            "total_tokens": 0,
        ...
        for _ in range(try_times):
            with httpx.Client(timeout=args.ray_request_timeout) as client:
                r = client.post(f"{args.ray_base_url}/call_llm", json=payload)
                if r.status_code != 200:
                    # likely due to the timeout of the ray service or internal error
                    this_task_logger.error("Failed to call LLM: " + r.text)
                    this_task_logger.error("Retrying...")
                    time.sleep(5)
                else:
                    data = r.json()
                    response = data.get("response", "")
                    usage = data.get("usage", _usage)
                    # generation failed
                    if response == "":
                        error = data.get("error", "")
                        error_message = error.get("message", "")
                        error_trace = error.get("trace", "")
                        this_task_logger.error(f"Response is empty\nerror message: {error_message}\nerror trace: {error_trace}")
                        time.sleep(5)
                        ...
                        if usage == {}:
                            usage = _usage
                            this_task_logger.warning("Usage is empty, using default usage")
                        return response, usage
        
        # if all retries failed, return the empty response and usage
        return "", _usage