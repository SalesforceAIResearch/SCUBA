...
from agents.s2_5.core.mllm import LMMAgent
...
class OSWorldACI(ACI):
    ...
    def generate_coords(self, ref_expr: str, obs: Dict) -> List[int]:
        ...
        # hancked the vllm grounding model response
        ...
        return [int(numericals[0]), int(numericals[1])], usage
    ...
    def assign_coordinates(self, plan: str, obs: Dict):
        ...
        if (
            ...
            self.coords1, total_usage = self.generate_coords(args[0], obs)
        # arg0 and arg1 are descriptions
        elif function_name == "agent.drag_and_drop" and len(args) >= 2:
            self.coords1, usage_1 = self.generate_coords(args[0], obs)
            self.coords2, usage_2 = self.generate_coords(args[1], obs)
            total_usage = {
