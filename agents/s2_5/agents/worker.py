...
from agents.s2_5.agents.grounding import ACI
from agents.s2_5.core.module import BaseModule
from agents.s2_5.memory.procedural_memory import PROCEDURAL_MEMORY
from agents.s2_5.utils.common_utils import (
    call_llm_safe,
    ...
    def generate_next_action(
        ...
        if self.enable_reflection:
            ...
            # Load the latest action
                ...
                if not isinstance(full_reflection, str):
                    reflection_usage = full_reflection.usage.to_dict()
                    full_reflection = full_reflection.choices[0].message.content
        ...
        if not isinstance(full_plan, str):
            plan_usage = full_plan.usage.to_dict()
            full_plan = full_plan.choices[0].message.content
        ...
        try:
            grounding_usage = agent.assign_coordinates(plan, obs)
        ...
        total_usage = {
            "prompt_tokens": grounding_usage["prompt_tokens"] + plan_usage["prompt_tokens"] + reflection_usage["prompt_tokens"],
            "completion_tokens": grounding_usage["completion_tokens"] + plan_usage["completion_tokens"] + reflection_usage["completion_tokens"],
            "total_tokens": grounding_usage["total_tokens"] + plan_usage["total_tokens"] + reflection_usage["total_tokens"],
        }
