...
class AnthropicAgent:
    def __init__(self,
        ...
        if provider == "vertex":
            self.provider = APIProvider.VERTEX
        elif provider == "bedrock":
            self.provider = APIProvider.BEDROCK
    ...
    def predict(self, task_instruction: str, obs: Dict = None, system: Any = None):
        ...
        if self.provider == APIProvider.ANTHROPIC:
            ...
            assert os.getenv('GOOGLE_APPLICATION_CREDENTIALS') is not None, "GOOGLE_APPLICATION_CREDENTIALS is not set"
            assert os.getenv('GOOGLE_PROJECT_ID') is not None, "GOOGLE_PROJECT_ID is not set"
            assert os.getenv('GOOGLE_REGION') is not None, "GOOGLE_REGION is not set"
            client = AnthropicVertex(project_id=os.getenv('GOOGLE_PROJECT_ID'), region=os.getenv('GOOGLE_REGION'))
        ...
        logger.debug(f"Received response params: {response_params}")
        ...
        try:
            claude_usage = response.usage.to_dict()
            usage["prompt_tokens"] = claude_usage["input_tokens"]
            usage["completion_tokens"] = claude_usage["output_tokens"]
            usage["total_tokens"] = claude_usage["input_tokens"] + claude_usage["output_tokens"]
        except Exception as e:
        ...
        for parse_retry in range(max_parse_retry):
            ...
            try:
                for content_block in response_params:
                    ...
                    elif content_block["type"] in ["text", "thinking"]:
                        ctype = content_block["type"]
                        reasonings.append({ctype: content_block[ctype]})
                if isinstance(reasonings, list) and len(reasonings) > 0:
                    # reasonings = reasonings[0]
                    # keep all the reasonings
                ...
                if len(actions) == 0:
                    actions = ["DONE"]
                return reasonings, actions, usage
            except Exception as e:
    ...
    def reset(self, _logger = None, *args, **kwargs):
        ...
        if _logger:
            ...
            logger = logging.getLogger("desktopenv.agent")
        self.messages = []
        logger.debug(f"{self.class_name} reset.")