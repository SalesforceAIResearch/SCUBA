...
class MessageHistory(BaseModel):
	...
	def remove_last_plan(self) -> None:
		"""Remove last plan from history"""
		either_ai_or_human_message = isinstance(self.messages[-1].message, AIMessage) or isinstance(self.messages[-1].message, HumanMessage)
		if len(self.messages) > 2 and either_ai_or_human_message:
			self.current_tokens -= self.messages[-1].metadata.tokens
			self.messages.pop()	
