...
from browser_use.custom.browser_context_zoo import BrowserContextBugFix, BrowserSessionBugFix
...
FORCE_RESET_PER_SETUP_CALL = os.environ.get(
    "FORCE_RESET_PER_SETUP_CALL", "False").lower() == "true"
...
class VWABrowser(BrowserBugFix):
...
@dataclass
class VWABrowserContextConfig(BrowserContextConfig):
    gelocation: Optional[str] = None
    storage_state: Optional[str] = None


class VWABrowserContext(BrowserContextBugFix):
...
class VWAEnv:
    def __init__(self,
                 browser: VWABrowser,
                 context: VWABrowserContext) -> None:
    ...
    async def asetup(self,  config_file: Path | None = None) -> None:
        ...
        if instance_config.get("require_reset", False):
            if FORCE_RESET_PER_SETUP_CALL:
                ...
                print(
                    f"skipping reset with {FORCE_RESET_PER_SETUP_CALL=} inside areset. Please handle this in an external script.")
