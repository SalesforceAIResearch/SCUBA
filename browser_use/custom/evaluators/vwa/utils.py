...
from browser_use.custom.evaluators.vwa.constants import OPENAI_API_BASE
from browser_use.custom.evaluators.vwa.openai_utils import generate_from_openai_chat_completion
...
from browser_use.custom.evaluators.vwa.env_config import (
    ACCOUNTS,
    REDDIT,
    SHOPPING,
    WIKIPEDIA,
...
async def shopping_get_latest_order_url() -> str:
...
async def shopping_get_sku_latest_review_author(sku: str) -> str:
...
async def shopping_get_sku_latest_review_rating(sku: str) -> str:
    """Get the latest review for shopping admin."""
    header = {
        "Authorization": f"Bearer {shopping_get_auth_token()}",
        "Content-Type": "application/json",
...
async def shopping_get_sku_latest_review_text(sku: str) -> str:
...
async def shopping_get_sku_latest_review_title(sku: str) -> str:
...
async def shopping_get_sku_product_page_url(sku: str) -> str:
...
async def reddit_get_post_url(url: str) -> str:
    """Get the post url"""
...
def llm_fuzzy_match(pred: str, reference: str, question: str) -> float:
    ...
    if "partially correct" in response or "incorrect" in response:
        ...
        try:
            ...
            # the output does not contain 'correct', 'incorrect', or 'partially correct'
            return 0.0
