...
from browser_use.custom.evaluators.vwa import image_utils
from browser_use.custom.evaluators.vwa.utils import (
...
class StringEvaluator(AEvaluator):
    ...
    def must_include(ref: str, pred: str) -> float:
        ...
        if len(word_tokenize(clean_ref)) == 1:
            ...
            # return float(clean_ref in tok_pred)  # clean_ref='1' will fail to match tok_pred=['1.00', '-', '5.00']
            return float(any([StringEvaluator._num_safe_match(clean_ref, tok) for tok in tok_pred]))
        else:
