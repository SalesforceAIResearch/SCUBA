...
def create_llm(provider, model_name, temperature=1.0, random_seed=42):
    if provider == "openai":
        if 'o3' not in model_name and 'gpt-5' not in model_name:
            llm = ChatOpenAI(model=model_name, temperature=temperature, seed=random_seed)
        elif 'o3' in model_name:
            llm = ChatOpenAI(model=model_name, seed=random_seed, temperature=1.0, disabled_params={'parallel_tool_calls': None})
        else:
        ...
        project_id='salesforce-research-internal'
        region="us-east5"
        ...
        llm = ChatAnthropicVertex(model=model_name, project=project_id, location=region, 
...
def get_mp4(src_type, history_dir=None, history_filename=None, gif_dir=None, gif_filename=None, mp4_dir=None, mp4_file_name=None):
    font_size = 20
    title_font_size = 20
    goal_font_size = 20
    ...
    if src_type == "history":
        ...
        if images:
            ...
            for idx, img in enumerate(images):
                frame_path = os.path.join(temp_dir, f"{idx:05d}.png")  # Zero-padded filenames
                img.save(frame_path)
            ffmpeg_cmd = [
                "ffmpeg", "-y", "-framerate", str(fps), "-i",
                os.path.join(temp_dir, "%05d.png"),  # Input sequence pattern
                "-c:v", "libx264", "-pix_fmt", "yuv420p", "-filter:v", f"setpts={1/speed_factor}*PTS",
                "-r", str(fps),  # Ensure correct output frame rate
                output_path
            ]
    ...
    for step in trajectory_jsonfied:
        ...
        for usage in usage_list:
            if 'total_cost' in usage:
                ...
                summary['warnings'].add('No total cost found in the usage; the cost might be underestimated or the callback for the model provider is not used/implemented.')

            if 'prompt_tokens' in usage:
                summary['total_prompt_tokens'] += usage['prompt_tokens']
            else:
                summary['warnings'].add('No prompt tokens found in the usage; the cost might be underestimated or the callback for the model provider is not used/implemented.')
            
            if 'prompt_tokens_cached' in usage:
                summary['total_prompt_tokens_cached'] += usage['prompt_tokens_cached']
            else:
                summary['warnings'].add('No prompt tokens cached found in the usage; the cost might be underestimated or the callback for the model provider is not used/implemented.')
                
            if 'completion_tokens' in usage:
                summary['total_completion_tokens'] += usage['completion_tokens']
            else:
                summary['warnings'].add('No completion tokens found in the usage; the cost might be underestimated or the callback for the model provider is not used/implemented.')
            if 'reasoning_tokens' in usage:
                summary['reasoning_tokens'] += usage['reasoning_tokens']
            else:
                summary['warnings'].add('No reasoning tokens found in the usage; the cost might be underestimated or the callback for the model provider is not used/implemented.')
            if 'total_tokens' in usage:
                summary['total_tokens'] += usage['total_tokens']
            else:
                summary['warnings'].add('No total tokens found in the usage; the cost might be underestimated or the callback for the model provider is not used/implemented.')
