...
from browser_use.telemetry.views import AgentStepTelemetryEvent, AgentEndTelemetryEvent
from browser_use.agent.gif import create_history_gif
from browser_use.agent.prompts import AgentMessagePrompt, PlannerPrompt, SystemPrompt
from browser_use.agent.message_manager.utils import convert_input_messages, extract_json_from_model_output, save_conversation
from browser_use.controller.registry.views import ActionModel
from browser_use.custom.utils import pil_to_b64
...
def get_vertex_usage(response):
    try:
        usage = response.response_metadata['usage']
        return {
            'cache_creation_input_tokens': usage['cache_creation_input_tokens'],
            'cache_read_input_tokens': usage['cache_read_input_tokens'],
            'prompt_tokens_cached': usage['cache_creation_input_tokens'] + usage['cache_read_input_tokens'],
        ...
        # aligned with the test_langchain_vertex.py
        ...
        return {
            ...
            'prompt_tokens_cached': usage_metadata['input_token_details']['cache_creation'] + usage_metadata['input_token_details']['cache_read'],
            'total_cost': 0.0,
            'reasoning_tokens': 0
        }
...
class AgentWithMessageTracker(Agent):
    ...
    def _add_action_results(self, result: list[ActionResult]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    async def _run_planner(self):
        ...
        planner_messages = [
            ...
            *self._message_manager.get_messages()[1:],  # Use full message history except the first (system message)
        ]
        ...
        if 'openai' in provider.lower():
            ...
            usage = get_usage(cb)
        elif 'vertex' in provider.lower():
            response = await self.settings.planner_llm.ainvoke(planner_messages)
            usage = get_vertex_usage(response)
        else:
            response = await self.settings.planner_llm.ainvoke(planner_messages)
            usage = {}
    ...
    async def get_next_action(self, input_messages: list[BaseMessage]) -> Union[AgentOutput, Dict[str, Any]]:
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            ...
            self.trajectory_jsonfied[self.state.n_steps]['input_messages'] = {
                    ...
                    'token_count': sum([self.message_manager._count_tokens(m) for m in input_messages]),
                    'cumulative_input_token_count': tokens,
                }

            try:
                # llm call 
                model_output, get_next_action_usage, logged_res = await self.get_next_action(input_messages)
                ...
                self.trajectory_jsonfied[self.state.n_steps-1]['output_messages'] = {
                    ...
                    # as a backup in case the callback context is not available
                    'token_count': sum([self.message_manager._count_tokens(m) for m in message_history[-2:]])
                }
            except Exception as e:
            ...
            result: list[ActionResult] = await self.multi_act(model_output.action, check_for_new_elements=False)
            ...
            # the self.state.n_steps was increased after the get_next_action call
            self.trajectory_jsonfied[self.state.n_steps-1]['controller_messages'] = self._add_action_results(result)
    ...
    async def run(self, max_steps: int = 100) -> Tuple[AgentHistoryList, Page]:
        ...
        try:
            ...
            if self.initial_actions:
                ...
                self.trajectory_jsonfied[0] = {'controller_messages': self._add_action_results(result)}
                self.state.last_result = result
            ...
            try:
                ...
                logger.info(f'Total tabs opened after initial actions: {self.total_tabs_opened}')
            except Exception as e:
                logger.info(f'Error getting total tabs opened after initial actions: {e}; likely there is no browser context as initial actions are not provided')
                self.total_tabs_opened = 0
...
class AgentWithCustomPlanner(AgentWithMessageTracker):
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.use_budget = kwargs.pop('use_budget', False)
        self.budegt = kwargs.pop('budget', -1)
        self.input_token_price_per_million = kwargs.pop('input_token_price_per_million')
        self.output_token_price_per_million = kwargs.pop('output_token_price_per_million')
        self.total_input_tokens = 0
        self.total_output_tokens = 0
        self.cost_for_budget = 0
    ...
    def _flatten_messages(self, messages: list[BaseMessage], omit_system_message: bool = False) -> str:
        text = ''
        for msg in messages:
            role = msg.type
    ...
    async def _run_planner(self):
        if not self.settings.planner_llm:
            return None
        from browser_use.custom.prompts import planner_system_prompt
        system_message = SystemMessage(content=planner_system_prompt)
        retrived_narrative_memory_list = self.planner_inputs.get('retrived_narrative_memory', [])
        ...
        if self.planner_inputs.get('include_init_state_observation', True):
            planner_messages = [
                ...
                *self._message_manager.get_messages()[5:],  # task history memory + browser state + last action result
            ]
        else:
            planner_messages = [
                system_message,
        ...
        if self.plan_history:
            planner_messages.append(HumanMessage(content="## previous plan for this task made by you\n" + self.plan_history[-1]))
        if not self.settings.use_vision_for_planner and self.settings.use_vision:
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            if self.use_budget:
                logger.info(f'Current cost {self.cost_for_budget}, budget {self.budegt}')
            ...
            if self.state.last_result:
                condtion_2 = self.state.last_result[-1].extracted_content == "Re-plan signal received"
            else:
                condtion_2 = False
            ...
            if self.settings.planner_llm and flag:
                ...
                if self.use_budget:
                    self.total_input_tokens += planner_usage['prompt_tokens']
                    output_tokens_ = planner_usage['total_tokens'] - planner_usage['prompt_tokens']
                    self.total_output_tokens += output_tokens_
                    self.cost_for_budget += self.total_input_tokens * self.input_token_price_per_million / 1000000.0 + self.total_output_tokens * self.output_token_price_per_million / 1000000.0
            ...
            # not the time to make a new plan; add the last plan again back to the history for the action prediction
                ...
                logger.info('🗒️ Planner: Not the time to make a new plan, so the last plan will be used again')
                self._message_manager.add_plan(self.plan_history[-1], position=-1, as_ai_message=False)
            ...
            if self.use_budget and self.cost_for_budget >= self.budegt:
                logger.info(f'Current cost {self.cost_for_budget} exceeded the budget {self.budegt}, stopping the agent.')
                msg = f'Current cost {self.cost_for_budget} for budget exceeded, stopping the agent. Use only the "done" action now. No other actions - so here your action sequence must have length 1.'
                msg += '\nIf the task is not yet fully finished as requested by the user, set success in "done" to false! E.g. if not all steps are fully completed.'
                msg += '\nIf the task is fully finished, set success in "done" to true.'
                msg += '\nInclude everything you found out for the ultimate task in the done text.'
                self._message_manager._add_message_with_tokens(HumanMessage(content=msg))
            ...
            try:
                ...
                if self.use_budget:
                    self.total_input_tokens += get_next_action_usage['prompt_tokens']
                    output_tokens_ = get_next_action_usage['total_tokens'] - get_next_action_usage['prompt_tokens']
                    self.total_output_tokens += output_tokens_
                    self.cost_for_budget = self.total_input_tokens * self.input_token_price_per_million / 1000000.0 + self.total_output_tokens * self.output_token_price_per_million / 1000000.0
...
from browser_use.custom.message_manager_zoo import PlainMessageManager                
...
class LLMAgent:
    ...
    async def aget_response(self, instruction: str, reset_message_manager: bool = False):
        ...
        if 'openai' in self.provider.lower():
            ...
            usage = get_usage(cb)
        elif 'vertex' in self.provider.lower():
            response = await self.llm.ainvoke(llm_input)
            usage = get_vertex_usage(response)
        else:
            response = await self.llm.ainvoke(llm_input)
...
from browser_use.custom.message_manager_zoo import MessageManagerWithImages
from browser_use.agent.message_manager.service import MessageManagerSettings
from browser_use.custom.eval_envs.VWA import VWABrowser, VWABrowserContext
...
class VWA_Agent(AgentWithCustomPlanner):
    def __init__(
        ...
        self.browser = browser if browser is not None else (None if browser_context else BrowserBugFix())
        if browser_context:
            self.browser_context = browser_context
        elif self.browser:
            self.browser_context = BrowserContextBugFix(browser=self.browser, config=self.browser.config.new_context_config)
        else:
